...
- `demo-machine analyze <output-dir>` generates analyzer artifacts for an existing run without recapture or rerender. It writes `review-bundle.json`, `review-prompt.md`, `video.shots.json`, `segment.evidence.json`, `layout-safety.report.json`, `demo-capture-evidence.json` when capture evidence exists, and `segment-storyboard/` files next to the run output.
