...
Set `DEMO_MACHINE_PUBLIC_SAFE=true` for sensitive public recordings where Playwright traces could retain tokens.
...
Generates `viewer.html` and `viewer.manifest.json` beside an existing
`output.mp4`, reading the completed run's events and metadata for chapter
timings and probing the rendered duration with FFprobe. A normal `run` also
generates these files automatically when the spec contains an enabled `share`
configuration. See the [Share Viewer](share-viewer.md).

`--no-narration` skips audio synthesis but does not remove reviewed text from a
share viewer: when no VTT is supplied, step narration and captured timestamps
produce the local `subtitles.vtt` track and clickable transcript.
...
- `verification.json`
- `quality.json`
- `trace.zip` unless `DEMO_MACHINE_PUBLIC_SAFE=true` is set
- `screenshots/manifest.json`
- `viewer.html` when `share` is configured
- `viewer.manifest.json` when `share` is configured
