...
describe("checkChapterTitles", () => {
  ...
  it("passes when chapter title screenshots have consistent styling", () => {
    // Same background, similar appearance — identical images pass easily
    const img = solidPng(10, 10, 30, 30, 40);
    const screenshots = new Map<number, Buffer>();
  ...
  it("fails when chapter title screenshots differ significantly", () => {
    const dark = solidPng(10, 10, 20, 20, 30);
  ...
  it("compares pairs in order by chapter index", () => {
    ...
    screenshots.set(5, c);
    const results = checkChapterTitles(baseCtx({ chapterTitleScreenshots: screenshots }));
    // Should compare 1→3 and 3→5
    const fails = results.filter((r) => r.status === "fail");
    expect(fails.length).toBeGreaterThanOrEqual(1);
    expect(fails.some((f) => f.message.includes("1→3"))).toBe(true);
  });

  it("includes suggestion about drawtext rendering", () => {
    const dark = solidPng(10, 10, 0, 0, 0);
    const bright = solidPng(10, 10, 255, 255, 255);
    const screenshots = new Map<number, Buffer>();
    screenshots.set(0, dark);
    screenshots.set(1, bright);
    const results = checkChapterTitles(baseCtx({ chapterTitleScreenshots: screenshots }));
    const fail = results.find((r) => r.status === "fail");
    expect(fail!.suggestion).toContain("drawtext");
  });
