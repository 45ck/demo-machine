...
const suites: ExampleSuite[] = [
  {
    ...
    canonicalSpec: "examples/dashboard-table.demo.yaml",
    variantSpecs: ["examples/dashboard-table-redaction.demo.yaml"],
    releaseTier: "nightly",
  ...
  {
    slug: "file-uploader",
    canonicalSpec: "examples/file-uploader.demo.yaml",
    variantSpecs: [],
    releaseTier: "pr",
    visualBaseline: "gallery",
...
describe("examples CLI helpers", () => {
  ...
  it("formats examples as a human-readable table", () => {
    ...
    expect(output).toContain("examples/file-uploader.demo.yaml");
  });
  ...
  it("finds examples by slug and formats details with runnable commands", () => {
    ...
    expect(formatExampleDetails(suite!)).toContain(
      "demo-machine run examples/dashboard-table.demo.yaml --no-headless",
    );
  });
